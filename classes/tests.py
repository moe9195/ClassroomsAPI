...
class ClassroomListViewTest(APITestCase):
	def setUp(self):
		...
		classroom1 = {'name': 'foo.jpg', 'subject': "yaaay", 'year': 4, 'teacher': user}
		classroom2 = {'name': 'foo.jpg', 'subject':"booo" , 'year': 5, 'teacher': user}
	...
	def test_serializer(self):
		...
		for index, classroom in enumerate(classrooms):
			self.assertEqual(classroom.subject, response.data[index]['subject'])
			self.assertEqual(classroom.year, response.data[index]['year'])
...
class ClassroomDetailViewTest(APITestCase):
	...
	def test_serializer(self):
		...
		self.assertEqual(classroom.subject, response.data['subject'])
...
class ClassroomCRUDTest(APITestCase):
	def setUp(self):
		user = User.objects.create_user(username="laila", password="1234567890-=")
		classroom1 = {'name': 'foo', 'subject': "yaaay", 'year': 4, 'teacher': user}
		classroom2 = {'name': 'fooj', 'subject':"booo" , 'year': 5, 'teacher': user}
		Classroom.objects.create(**classroom1)
		Classroom.objects.create(**classroom2)
		self.data = {'name': 'x', 'subject': "y", 'year': 8}
	...
	def test_authorized_create(self):
		...
		self.assertEqual(classrooms.last().subject, self.data['subject'])
		self.assertEqual(classrooms.last().year, self.data['year'])
	...
	def test_update(self):
		...
		self.assertEqual(classroom.subject, self.data['subject'])
		self.assertEqual(classroom.year, self.data['year'])
	...
	def test_delete(self):
		...
		self.assertFalse(classroom_exists)
